...
if (mysqli_num_rows($result) != 1) { header("Location: tours.php"); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name)||empty($email)||empty($phone)||empty($travel_date)) { $booking_error = "Please fill in all required fields."; }
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) { $booking_error = "Please enter a valid email address."; }
    elseif ($num_people < 1 || $num_people > $tour['max_people']) { $booking_error = "Number of people must be between 1 and {$tour['max_people']}."; }
    elseif (strtotime($travel_date) < strtotime('+1 day')) { $booking_error = "Travel date must be at least 1 day from today."; }
    else {
        ...
        if (mysqli_query($conn, $sql)) { header("Location: booking-success.php?id=".mysqli_insert_id($conn)); exit; }
        else { $booking_error = "Booking failed. Please try again."; }
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        .tour-detail-layout { display:grid; grid-template-columns:1fr 380px; gap:48px; align-items:start; }
        .tour-hero-image { width:100%; height:520px; object-fit:cover; border-radius:var(--radius-lg); box-shadow:var(--shadow); margin-bottom:40px; }
        .tour-info-bar { display:flex; background:var(--white); border-radius:var(--radius-lg); box-shadow:var(--shadow); overflow:hidden; margin-bottom:40px; }
        .info-bar-item { flex:1; padding:24px 20px; text-align:center; border-right:1px solid var(--light-2); }
        .info-bar-item:last-child { border-right:none; }
        .info-bar-icon { font-size:1.5rem; margin-bottom:8px; display:block; }
        .info-bar-label { font-size:0.72rem; letter-spacing:1.5px; text-transform:uppercase; color:var(--gray); font-weight:700; display:block; margin-bottom:4px; }
        .info-bar-value { font-family:var(--font-display); font-size:1.1rem; color:var(--dark); font-weight:600; }
        .tour-content h2 { font-size:1.6rem; margin:36px 0 16px; padding-bottom:12px; border-bottom:2px solid var(--light-2); }
        .tour-full-content { line-height:1.9; font-size:0.95rem; color:var(--text-light); }
        .booking-card { background:var(--white); border-radius:var(--radius-lg); box-shadow:var(--shadow); overflow:hidden; position:sticky; top:100px; }
        .booking-card-header { background:var(--dark); padding:28px 30px; text-align:center; }
        .booking-card-header .price-label { font-size:0.75rem; letter-spacing:2px; text-transform:uppercase; color:rgba(255,255,255,0.5); margin-bottom:8px; }
        .booking-card-header .price { font-family:var(--font-display); font-size:2.2rem; color:var(--gold); display:block; }
        .booking-card-header .price-per { font-size:0.82rem; color:rgba(255,255,255,0.5); }
        .booking-form { padding:30px; }
        .booking-form h3 { font-size:1.1rem; margin-bottom:24px; padding-bottom:14px; border-bottom:1px solid var(--light-2); }
        .price-summary { background:var(--light); border-radius:var(--radius); padding:16px; margin:20px 0; }
        .price-row { display:flex; justify-content:space-between; font-size:0.88rem; margin-bottom:8px; color:var(--text-light); }
        .price-row.total { margin-top:12px; padding-top:12px; border-top:1px solid var(--light-2); font-weight:700; color:var(--dark); font-size:1rem; }
        .price-row.total .amount { color:var(--gold-dark); font-family:var(--font-display); }
        .related-section { margin-top:70px; padding-top:50px; border-top:2px solid var(--light-2); }
        @media(max-width:960px){ .tour-detail-layout { grid-template-columns:1fr; } .booking-card { position:static; } }
        @media(max-width:640px){ .tour-info-bar { flex-wrap:wrap; } .info-bar-item { flex:1 1 50%; border-bottom:1px solid var(--light-2); } }
    </style>
</head>
<body>
...
<section class="section" style="padding-top:50px;">
    <div class="container">
        <img src="<?php echo htmlspecialchars($tour['image_url']); ?>" alt="<?php echo htmlspecialchars($tour['title']); ?>" class="tour-hero-image">
                    ...
                    <div class="booking-form">
                        ...
                        <form method="POST" action="tour-detail.php?id=<?php echo $tour_id; ?>#booking" id="bookingForm">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="travel_date" class="form-control" required id="travelDate" min="<?php echo date('Y-m-d', strtotime('+1 day')); ?>" value="<?php echo isset($_POST['travel_date'])?htmlspecialchars($_POST['travel_date']):''; ?>">
                            </div>
                            <div class="form-group">
            ...
            <div class="tours-grid">
                ...
                <div class="tour-card">
                    <div class="tour-card-image">
                        <img src="<?php echo htmlspecialchars($rel['image_url']); ?>" alt="<?php echo htmlspecialchars($rel['title']); ?>" loading="lazy">
...
<script>
    ...
    document.getElementById('bookingForm').addEventListener('submit', function(e) {
        const name=this.full_name.value.trim(), email=this.email.value.trim(), phone=this.phone.value.trim(), date=this.travel_date.value, n=parseInt(this.num_people.value);
        if (!name||!email||!phone||!date){e.preventDefault();alert('Please fill in all required fields.');return;}
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){e.preventDefault();alert('Invalid email address.');return;}
        if (n<1||n><?php echo $tour['max_people']; ?>){e.preventDefault();alert('Invalid number of people.');return;}
    });
</script>
</body>
</html>
