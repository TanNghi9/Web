...
<head>
    ...
    <style>
        .hero { height:100vh; min-height:700px; background:url('https://media.thuonghieucongluan.vn/uploads/2024/11/25/du-lich-viet-nam-1732534409.jpg') center/cover no-repeat; position:relative; display:flex; align-items:center; }
        .hero::before { content:''; position:absolute; inset:0; background:linear-gradient(to right,rgba(13,13,13,0.88) 40%,rgba(13,13,13,0.3)); }
        .hero-content { position:relative; z-index:1; max-width:640px; }
        .hero-label { font-size:0.75rem; letter-spacing:4px; text-transform:uppercase; color:var(--gold); font-weight:700; margin-bottom:20px; display:block; }
        .hero h1 { font-size:clamp(2.5rem,5vw,4.5rem); color:var(--white); line-height:1.1; margin-bottom:24px; }
        .hero h1 em { color:var(--gold); font-style:italic; }
        .hero p { font-size:1.1rem; color:rgba(255,255,255,0.75); margin-bottom:40px; max-width:480px; line-height:1.8; }
        .hero-buttons { display:flex; gap:16px; flex-wrap:wrap; }
        .search-wrap { margin-top:-60px; position:relative; z-index:10; }
        .search-box { background:var(--white); border-radius:var(--radius-lg); box-shadow:0 20px 60px rgba(0,0,0,0.15); padding:36px 40px; display:grid; grid-template-columns:1fr 1fr 1fr auto; gap:20px; align-items:end; }
        .search-box .form-group { margin:0; }
        @media(max-width:900px){ .search-box { grid-template-columns:1fr 1fr; } }
        @media(max-width:580px){ .search-box { grid-template-columns:1fr; padding:24px; } .search-wrap { margin-top:0; } }
        .stats-section { background:var(--dark); padding:60px 0; }
        .stats-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:40px; text-align:center; }
        .stat-item .number { font-family:var(--font-display); font-size:2.8rem; color:var(--gold); display:block; line-height:1; margin-bottom:8px; }
        .stat-item .label { font-size:0.82rem; letter-spacing:2px; text-transform:uppercase; color:rgba(255,255,255,0.5); font-weight:600; }
        ...
        .cat-card { background:var(--white); border-radius:var(--radius-lg); padding:36px 28px; text-align:center; border:1px solid var(--light-2); transition:var(--transition); display:block; }
        .cat-card:hover { border-color:var(--gold); box-shadow:var(--shadow-gold); transform:translateY(-4px); color:inherit; }
        .cat-icon { font-size:2.5rem; margin-bottom:16px; display:block; }
        .cat-card h3 { font-size:1.1rem; color:var(--dark); margin-bottom:8px; }
        .cat-card p { font-size:0.82rem; color:var(--text-light); margin:0 0 12px; }
        .cat-count { font-size:0.78rem; color:var(--gold); font-weight:700; letter-spacing:1px; }
        ...
        .why-icon { width:72px; height:72px; background:rgba(201,168,76,0.12); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.8rem; margin:0 auto 20px; border:1px solid rgba(201,168,76,0.25); }
        .why-item h4 { color:var(--white); margin-bottom:10px; font-size:1rem; }
        .why-item p { color:rgba(255,255,255,0.45); font-size:0.88rem; margin:0; }
        @media(max-width:768px){ .why-grid { grid-template-columns:repeat(2,1fr); gap:28px; } }
        .cta-section { background:url('https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=1600') center/cover no-repeat; position:relative; padding:120px 0; text-align:center; }
        .cta-section::before { content:''; position:absolute; inset:0; background:rgba(13,13,13,0.75); }
        .cta-content { position:relative; z-index:1; max-width:640px; margin:0 auto; }
        .cta-content h2 { color:var(--white); margin-bottom:16px; }
        .cta-content p { color:rgba(255,255,255,0.7); margin-bottom:36px; font-size:1.05rem; }
    </style>
</head>
<body>
...
<section class="stats-section">
    <div class="container">
        <div class="stats-grid">
            <div class="stat-item"><span class="number">63+</span><span class="label">Destinations</span></div>
            <div class="stat-item"><span class="number">12K+</span><span class="label">Happy Travelers</span></div>
            <div class="stat-item"><span class="number">15+</span><span class="label">Years Experience</span></div>
            <div class="stat-item"><span class="number">98%</span><span class="label">Satisfaction Rate</span></div>
        </div>
    </div>
</section>
...
<section class="section">
    <div class="container">
        <div class="section-header">
            ...
            <h2>Latest Tours</h2>
            <p>Freshly curated journeys added to our collection of extraordinary experiences.</p>
            <div class="gold-line"></div>
        </div>
        <div class="tours-grid">
            <?php while ($tour = mysqli_fetch_assoc($latest_result)): ?>
            <div class="tour-card">
                <div class="tour-card-image">
                    <img src="<?php echo htmlspecialchars($tour['image_url']); ?>" alt="<?php echo htmlspecialchars($tour['title']); ?>">
                    <span class="tour-card-badge"><?php echo htmlspecialchars($tour['cat_name']); ?></span>
...
<section class="section why-section" id="why-us">
    <div class="container">
        <div class="section-header">
            ...
            <h2 style="color:var(--white);">The VietNam Travel Difference</h2>
            <p style="color:rgba(255,255,255,0.5);">We go beyond travel — we craft transformative journeys.</p>
            <div class="gold-line"></div>
        </div>
        <div class="why-grid">
            <div class="why-item"><div class="why-icon">🏆</div><h4>Award-Winning Service</h4><p>Recognized by leading travel publications for excellence and innovation.</p></div>
            <div class="why-item"><div class="why-icon">🔒</div><h4>Safe & Secure</h4><p>Your safety is our priority with 24/7 support and comprehensive travel insurance.</p></div>
            <div class="why-item"><div class="why-icon">🌿</div><h4>Responsible Travel</h4><p>Committed to sustainable tourism that preserves destinations for future generations.</p></div>
            <div class="why-item"><div class="why-icon">✨</div><h4>Luxury Curated</h4><p>Every detail meticulously crafted for the most discerning travelers.</p></div>
        </div>
    </div>
</section>
